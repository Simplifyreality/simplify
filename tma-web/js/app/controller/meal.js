angular.module('tmweb').controller('MealController', ['$rootScope', '$scope', '$location', '$routeParams', '$filter', 'BoardingService', 'FiltersTransmissionService', function($rootScope, $scope, $location, $routeParams, $filter, BoardingService, FiltersTransmissionService) {
	...
	$scope.list = function() {
		...
		BoardingService.listBoarding(function(data) {
			$scope.results = data;
		}, function(error) {
	...
	$scope.exportUrl = function() {
		var url = $rootScope.api + '/boarding/export';
		var i = 0;
  ...
  FiltersTransmissionService.setFilter('mealControllers', $location.url());
}]);
