angular.module('tmweb').controller('JourneyLogsMomentumController', ['$rootScope', '$scope', '$route', '$location', '$routeParams', '$filter', 'JourneyService', 'FiltersTransmissionService', function($rootScope, $scope, $route, $location, $routeParams, $filter, journeyService, FiltersTransmissionService) {
	...
	$scope.exportUrl = function() {
		var url = $rootScope.api + '/momentum_logs/export';
		var i = 0;
  ...
  $scope.refreshJourneyLog = function () {
    $route.reload();
  }

  FiltersTransmissionService.setFilter('journeyControllers', $location.url());
}]);
