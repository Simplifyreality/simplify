angular.module('tmweb').controller('FaultHistoryController', ['$rootScope', '$scope', '$location', '$routeParams', '$filter', '$interval', '$translate', '$route', 'FaultService', 'FiltersTransmissionService', function($rootScope, $scope, $location, $routeParams, $filter, $interval, $translate, $route, faultService, FiltersTransmissionService) {
	...
	$scope.list = function(page) {
		...
		if ($routeParams.dateFrom != null) {
			...
			if (!$routeParams.defaultDate) {
				let weekAgo = new Date();
				weekAgo.setDate(weekAgo.getDate() - 7);
        $scope.options.dateFrom = $filter('date')(weekAgo, 'dd-MM-yyyy');
        $scope.options.defaultDate = true;
			}
		}
		...
		if ($routeParams.depot === undefined || ($routeParams.depot && $routeParams.depot.toLowerCase() === "null")) {
			$scope.options.depot = "ALL";
		} else {
	...
	$scope.removeSelectedFilter = function(key, value) {
		...
		if (key === 'dateFrom')  $scope.options.defaultDate = true;
		$location.search($scope.options);
	};
	...
	$scope.applyFilters = function() {
		$scope.options = {};
		$scope.options['depot'] = $scope.tab == "ALL" ? null : $scope.tab;
		$scope.options['status'] = [];
		$scope.options['priority'] = [];
        $scope.options['severity'] = [];
        ...
        if ($scope.filters.depeche) {
            $scope.options['severity'].push('DEPECHE');
        }
        if ($scope.filters.nodepeche) {
            $scope.options['severity'].push('CARNET');
        }
  ...
  FiltersTransmissionService.setFilter('faultControllers', $location.url());
